...
int find_next_without_target(Process *proc, int N_procs,int time){
	int next = -1;
	int nexttime = INT_MAX;
	for (int i = 0; i < N_procs; i++){
		if (proc[i].exec_time && (proc[i].ready_time < nexttime) && (proc[i].pid < 0) && proc[i].ready_time>time){
...
int scheduler_PSJF(Process *proc, int N_procs){
	...
	while (finish < N_procs){
		...
		if (target != -1){
			...
			int next = find_next_without_target(proc, N_procs,time);
